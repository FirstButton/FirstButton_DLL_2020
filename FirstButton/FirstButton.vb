...
'-------------------------------------------------------------------------------------------'
' COPYRIGHT © 2005-2008, 2010-2015, 2020  LIQUIDITY LIGHTHOUSE, LLC. ALL RIGHTS RESERVED.   '
' PORTIONS ARE CONVERTED FROM CODE THAT IS COPYRIGHT © 2005-2008  LIQUIDITY LIGHTHOUSE, LLC.'
'-------------------------------------------------------------------------------------------'
...
Public Class FBClass1
    ...
    Public logo_use_skipahead_logic As Boolean
    Public logo_IE7_use_skipahead_logic As Boolean
...
delete_exit:
    ...
    Public Class myReverserClass
        ...
        End Function
    End Class
Dim logo_cmdmap_spread_above As Integer
Dim logo_cmdmap_spread_below As Integer
Dim logo_current_nextid_value As Integer
Dim logo_current_nextid_inuse As Boolean
Dim logo_nextid_spread_changed As Boolean
...
Public win_vista_flag As String
Public win7_flag As String
Public win8_flag As String
Public winverflag As String
Dim win_UAC_enabled_flag As Boolean
Dim logo_check_UAC_flag As Boolean
...
Private Sub initialize_settings()
 ...
 End If  'end of check for install activity being true'
    ...
    logo_check_UAC_flag = False
    getversion_somb()
    get_IE_version()

 '   If logo_check_UAC_flag = True Then    
    'check registry to see if UAC is enabled'
 '     check_UAC_setting()                 
 '   End If
 '   If win_UAC_enabled_flag = True Then   
 ...
 '      MsgBoxStyle.Information, "FirstButton  Installer")          
 '   End If                                
...
Private Sub check_nextid()
  ...
  logo_current_nextid_inuse = False
  logo_sub_keyname = "Software\Microsoft\Internet Explorer\Extensions\CmdMapping"
    ...
    End Try
         ...
         If logo_string_ret_length >= 1 Then
          ...
          If logo_next_avail_id < 8192 Or _
             logo_next_avail_id > 8702 Then
               logo_nextid_missing_flag = True
          Else
             logo_current_nextid_value = logo_next_avail_id
          End If
         Else
          logo_nextid_missing_flag = True
         End If
     ...
     If logo_lreturnvalue = 2 _
      ...
      If logo_rqve_lpcvalues > 0 Then
          ...
          For Each valueName As String In rk2.GetValueNames()
           rev_lpvaluename = Space(200)
           rev_lpcbvaluename = 200
           If valueName.GetType.ToString = "System.String" Then
             ...
             Else
               If rk2.GetValue(valueName).GetType.ToString = "System.Int32" Then
                ...
                If logo_next_avail_id >= 8192 And logo_next_avail_id <= 8699 Then
                  logo_next_avail_id = logo_next_avail_id + 1
                  logo_cmdmap_spread_above = 8702 - logo_next_avail_id
                  logo_cmdmap_spread_below = logo_next_avail_id - 8192
                  If logo_cmdmap_spread_above >= logo_cmdmap_spread_below Then
                    logo_next_avail_id = logo_cmdmap_spread_above
     ...
     If mapcount > 0 Then
       ...
       For rev_dwindex = 0 To mapcount - 1
         If cmdmapping_array(rev_dwindex).cmd_valuedword > 0 Then
           cmdmapping_sort_array(rev_dwindex) = _
             cmdmapping_array(rev_dwindex).cmd_valuedword.ToString
           'check to see if the NextId value is already being used'
      ...
      'NextId value is already being used'
               logo_current_nextid_inuse = True
             End If
           End If
         End If
       Next
     ...
     End If
      ...
      If logo_high_dblword_value >= 0 Then
          If logo_high_dblword_value < 8702 And logo_high_dblword_value >= 8192 Then
             If mapcount > 1 Then
               If logo_current_nextid_inuse = True Then
                 logo_next_avail_id = logo_high_dblword_value + 1
                 logo_return_value = setcmdmapnum("NextId", logo_next_avail_id)
                 logo_nextid_missing_flag = False
               End If
             Else
               If mapcount = 1 Then
                  ...
                  If logo_cmdmap_spread_above >= logo_cmdmap_spread_below Then
                    If logo_high_dblword_value > 8192 Then
                      logo_next_avail_id = logo_high_dblword_value + 1
                      logo_return_value = setcmdmapnum("NextId", logo_next_avail_id)
                      logo_nextid_missing_flag = False
                      logo_nextid_spread_changed = True
                    Else
                      logo_next_avail_id = 8193
                      logo_return_value = setcmdmapnum("NextId", logo_next_avail_id)
                      logo_nextid_missing_flag = False
                      logo_nextid_spread_changed = True
                    End If
...
Private Sub get_IE_version()
  ...
  '   IE7_version_string = "1" Then   
  '    IE7_flag = True
  '    IE8_flag = True
  '    IE9_flag = True
  '    IE10_flag = True
  '    logo_IE_final_msg = "To display the new Toolbar in IE10 (desktop mode), select New Session from the File Menu, or select Internet Explorer from the Windows Start Menu."   
  'End If
 End If
End Sub
  ...
  If logo_rqve_lpcsubkeys > 0 Then
      ...
      If logo_rqve_lpcsubkeys > 0 Then
          ...
          For Each subKeyName As String In rk.GetSubKeyNames()
              ...
              '    rk.OpenSubKey(subKeyName)  - fixed bug of not processing hkcu extensions'
              If Mid(subKeyName.ToString, 1, 1) = "{" Then
                 cmd_ext_array(key_dwindex).ea_extension_name = subKeyName.ToString
                 If key_dwindex < logo_rqve_lpcsubkeys - 1 Then
                    key_dwindex = key_dwindex + 1
...
' added for UAC check in Vista, 7, 8, 10'
  Dim logo_string_ret_length_2 As Integer
...
Private Sub match_b4_IE7_extensions()
  ...
  'If cmdcount_2 > 1 Then'
   If cmdcount_2 >= 1 Then              'bug fix for when only 1 cmdmapping exists'
      For rev_dwindex = 0 To cmdcount_2 - 1
          rev_lpvaluename = Space(200)
          rev_lpcbvaluename = 38
...
Private Sub install_reg_entries()
  ...
  logo_IE7_lreturnvalue = 0
  logo_create_rc = 0
  logo_nextid_spread_changed = False
  locate_favorites()
       ...
       If logo_rqve_lpcvalues > 1 Then
        ...
        For Each valueName As String In rk.GetValueNames()
          ...
          If valueName.GetType.ToString = "System.String" Then
              rev_lpvaluename = valueName.ToString
          End If
          If logo_rqve_lpcvalues > 89 And cmdcount > 89 Then
            Exit For
          End If
          If Mid(logo_new_guid, 1, 38) = Mid(rev_lpvaluename, 1, 38) Then
              logo_install_guid = "Y"
      ...
      Else
        If cmdcount = 1 Then
            ...
            cmdarray(0).ce_valuedword <= 8696 Then
              If logo_nextid_spread_changed = False Then
